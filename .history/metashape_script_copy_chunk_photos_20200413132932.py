...
class CopyChunkPhotosDlg(QtWidgets.QDialog):
    ...
    def check_path(self, chunk):
        folders_to_ignore = []
        source1 = chunk.cameras[0].photo.path
        path_without_drive1 = os.path.splitdrive(source1)[1]
        subfolder1 = os.path.splitext(path_without_drive1)[0]
        path_to_photo1 = os.path.split(path_without_drive1)[0]
        folders_list1 = path_to_photo1.split('/')
        ...
        for c in chunk.cameras:
            ...
            folders_list = path_to_photo.split('/')
            check = any(item in folders_list for item in precedent_list)
            print(check)
            precedent_list = folders_list
            # print(folders_list)
